...
class CursorHbarsUse(SCPICmdWrite):
    ...
    **Description:**
        - Sets the horizontal bar cursor measurement scale. This command is only applicable when
          ratio cursors are on.
    ...
    **Info:**
        ...
        - ``HALFgrat`` resets the H bar measurement scale to half the number of divisions (five for
          some models and four for others) so that 25% is the current position of the lowest H Bar
          cursor and 75% is the current position of the highest H Bar.
        - ``FIVEdivs`` sets H Bar measurement scale so that five screen major divisions is 100%,
    ...
    **SCPI Syntax:**
    ...
    def use(self) -> CursorHbarsUse:
        ...
        **Info:**
            - ``CURrent`` sets the H Bar measurement scale so that 0% is the current position of the
              lowest H Bar cursor and 100% is the current position of the highest H Bar cursor.
            - ``HALFgrat`` resets the H bar measurement scale to half the number of divisions (five
              for some models and four for others) so that 25% is the current position of the lowest
              H Bar cursor and 75% is the current position of the highest H Bar.
            - ``FIVEdivs`` sets H Bar measurement scale so that five screen major divisions is 100%,
              where 0% is -2.5 divisions and 100% is +2.5 divisions from the center horizontal
              graticule.
        """
...
class CursorFunction(SCPICmdWrite, SCPICmdRead):
    ...
    **Description:**
        - Sets or returns the cursor type. Cursors are attached to the selected waveform in Waveform
          mode and are attached to the display area in Screen mode.
    ...
    **Info:**
        ...
        - ``SCREEN`` specifies both horizontal and vertical bar cursors, which measure the selected
          waveform in horizontal and vertical units. Use these cursors to measure anywhere in the
        ...
        - ``WAVEform`` specifies paired cursors in YT display format for measuring waveform
          ...
          to the trigger.
        - ``VBArs`` specifies the vertical bar cursor to measure the selected waveform in vertical
          units.
