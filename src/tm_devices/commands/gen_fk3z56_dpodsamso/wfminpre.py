...
class WfminpreWfid(SCPICmdWrite):
    """The ``WFMInpre:WFId`` command.

    **Description:**
        - This command (no query form) accepts but ignores the argument. This command is provided
          only to allow a waveform extracted from the instrument to be easily imported.
...
class WfminpreNrFr(SCPICmdRead):
    """The ``WFMInpre:NR_FR`` command.

    **Description:**
        - This query returns the number of frames to be transferred to the instrument with the CURVe
          command.

    **Usage:**
        - Using the ``.query()`` method will send the ``WFMInpre:NR_FR?`` query.
        - Using the ``.verify(value)`` method will send the ``WFMInpre:NR_FR?`` query and raise an
          AssertionError if the returned value does not match ``value``.
...
class Wfminpre(SCPICmdRead):
    ...
    **SCPI Syntax:**
    ...
    def __init__(self, device: Optional["PIDevice"] = None, cmd_syntax: str = "WFMInpre") -> None:
        ...
        self._nr_fr = WfminpreNrFr(device, f"{self._cmd_syntax}:NR_FR")
        ...
        self._wfid = WfminpreWfid(device, f"{self._cmd_syntax}:WFId")
    ...
    @property
    def nr_fr(self) -> WfminpreNrFr:
        """Return the ``WFMInpre:NR_FR`` command.

        **Description:**
            - This query returns the number of frames to be transferred to the instrument with the
              CURVe command.

        **Usage:**
            - Using the ``.query()`` method will send the ``WFMInpre:NR_FR?`` query.
            - Using the ``.verify(value)`` method will send the ``WFMInpre:NR_FR?`` query and raise
              an AssertionError if the returned value does not match ``value``.
    ...
    @property
    def wfid(self) -> WfminpreWfid:
        """Return the ``WFMInpre:WFId`` command.

        **Description:**
            - This command (no query form) accepts but ignores the argument. This command is
              provided only to allow a waveform extracted from the instrument to be easily imported.
