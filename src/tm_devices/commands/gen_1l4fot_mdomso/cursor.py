...
class CursorSource(SCPICmdWrite, SCPICmdRead):
    """The ``CURSor:SOUrce`` command.

    Description:
        - This command specifies the cursor source waveform, which can be one of channels 1-4,
          reference waveforms 1-4, math waveform, bus 1-4 (1-3 for the MDO4000C and 1-2 for the
          MDO3000), digital channels 0-15 (MSO/MDO4000/B models only as well as MDO3000 and MDO400C
          models with option MDO3MSO or MDO4MSO installed), the RF time domain traces (MDO4000/B/C
          models), or AUTO. When the cursor source is set to AUTO, cursor readouts apply to the
          currently selected waveform.

    Usage:
        - Using the ``.query()`` method will send the ``CURSor:SOUrce?`` query.
        - Using the ``.verify(value)`` method will send the ``CURSor:SOUrce?`` query and raise an
          AssertionError if the returned value does not match ``value``.
    ...
    Info:
        ...
        - ``BUS<x>`` - specifies the cursor source as one of the bus waveforms. x has a minimum of 1
          and a maximum of 4.
        - ``D<x>`` - specifies the cursor source as one of the digital waveforms. (MSO/MDO4000/B
          models only as well as MDO3000 and MDO400C models with option MDO3MSO or MDO4MSO
          installed.) x has a minimum of 0 and a maximum of 15.
        - ``RF_AMPlitude`` - specifies the cursor source as one of the RF time domain traces
          (Amplitude vs. Time, Frequency vs. Time or Phase vs. Time). (MDO4000/B/C series models
          only.).
        - ``RF_FREQuency`` - specifies the cursor source as one of the RF time domain traces
          (Amplitude vs. Time, Frequency vs. Time or Phase vs. Time). (MDO4000/B/C series models
          only.).
        - ``RF_PHASe`` - specifies the cursor source as one of the RF time domain traces (Amplitude
          vs. Time, Frequency vs. Time or Phase vs. Time). (MDO4000/B/C series models only.).
...
class Cursor(SCPICmdRead):
    ...
    def __init__(self, device: Optional["PIDevice"] = None, cmd_syntax: str = "CURSor") -> None:
        ...
        self._source = CursorSource(device, f"{self._cmd_syntax}:SOUrce")
    ...
    @property
    def source(self) -> CursorSource:
        """Return the ``CURSor:SOUrce`` command.

        Description:
            - This command specifies the cursor source waveform, which can be one of channels 1-4,
              reference waveforms 1-4, math waveform, bus 1-4 (1-3 for the MDO4000C and 1-2 for the
              MDO3000), digital channels 0-15 (MSO/MDO4000/B models only as well as MDO3000 and
              MDO400C models with option MDO3MSO or MDO4MSO installed), the RF time domain traces
              (MDO4000/B/C models), or AUTO. When the cursor source is set to AUTO, cursor readouts
              apply to the currently selected waveform.

        Usage:
            - Using the ``.query()`` method will send the ``CURSor:SOUrce?`` query.
            - Using the ``.verify(value)`` method will send the ``CURSor:SOUrce?`` query and raise
              an AssertionError if the returned value does not match ``value``.
            - Using the ``.write(value)`` method will send the ``CURSor:SOUrce value`` command.
        ...
        Info:
            ...
            - ``RF_FREQuency`` - specifies the cursor source as one of the RF time domain traces
              (Amplitude vs. Time, Frequency vs. Time or Phase vs. Time). (MDO4000/B/C series models
              only.).
            - ``RF_PHASe`` - specifies the cursor source as one of the RF time domain traces
              (Amplitude vs. Time, Frequency vs. Time or Phase vs. Time). (MDO4000/B/C series models
              only.).
            - ``AUTO`` - specifies the currently selected waveform. The selected waveform can be
              determined by querying ``SELECT:CONTROL``.
