...
from .gen_4jiykk_dpo.channelmapping import Channelmapping
from .gen_4jiykk_dpo.counter import Counter
from .gen_4jiykk_dpo.errordetector import Errordetector
from .gen_4jiykk_dpo.idnmultiscope import Idnmultiscope
from .gen_4jiykk_dpo.linktraining import Linktraining
from .gen_4jiykk_dpo.rosc import Rosc
from .gen_4jiykk_dpo.trigger import Trigger
...
class DPO70KSXCommandConstants:
    """The DPO70KSX command argument constants.

    This provides access to all the string constants which can be used as arguments for DPO70KSX
    commands.
    """
    ...
    # LARGE = "LARge"
    LAST = "LAST"
    LAST_FRAME = "LAST_FRAME"  # LASt_frame
    LEARN = "LEARN"
...
class DPO70KSXCommands:
    """The DPO70KSX commands.

    This provides access to all the commands for the DPO70KSX device. See the documentation of each
    ...
    Properties:
        ...
        - ``.ch``: The ``CH<x>`` command.
        - ``.channelmapping``: The ``CHANNELMAPping`` command.
        - ``.clear``: The ``CLEAR`` command.
        - ``.cls``: The ``*CLS`` command.
        - ``.cmdbatch``: The ``CMDBatch`` command.
        - ``.counter``: The ``COUnter`` command.
        ...
        - ``.id``: The ``ID`` command.
        - ``.idn``: The ``*IDN`` command.
        - ``.idnmultiscope``: The ``IDNMultiscope`` command.
    ...
    def __init__(self, device: Optional[PIDevice] = None) -> None:  # noqa: PLR0915
        ...
        self._idnmultiscope = Idnmultiscope(device)
    ...
    @property
    def channelmapping(self) -> Channelmapping:
        """Return the ``CHANNELMAPping`` command.

        **Description:**
            - This query returns a list of all channel mappings (such as,
              CH1.B2,CH2.A2,CH3.C2,CH4.D2). DPO70000SX Series only.

        **Usage:**
            - Using the ``.query()`` method will send the ``CHANNELMAPping?`` query.
            - Using the ``.verify(value)`` method will send the ``CHANNELMAPping?`` query and raise
    ...
    def counter(self) -> Counter:
        """Return the ``COUnter`` command.

        **Description:**
            - This command is used for three separate purposes. DPO70000SX Series only. This command
              is optional. First, the ``COUnter REFerence`` command takes the reference to be the
              last measured value when the command is received. After that, the displayed values are
              deltas from the reference value. In linear terms reference is just the offset. Second,
              the ``COUnter CLEAR`` command zeroes both the measured and derived statistical values,
              and zeroes the number of acquisitions used to accumulate the statistics. Third, the
              ``COUnter RESET`` command is only needed in the Trigger Source Frequency Application
              to recalculate the A->B Sequence Trig on Nth Event value. This is only necessary if
              the user has overridden the automatically calculated Trig on Nth Event value. The
              application automatically calculates the Trig on Nth Event value to achieve an
              acquisition update rate of about 1/3 second, to both maximize the number of digits in
              the measured result, and keep the acquisition rate lively.
        ...
        **Info:**
            ...
            - ``CLEAR`` zeroes both the measured and derived statistical values, and zeroes the
              number of acquisitions used to accumulate the statistics.
    ...
    @property
    def idnmultiscope(self) -> Idnmultiscope:
        """Return the ``IDNMultiscope`` command.

        **Description:**
            - This query returns instrument data on all MultiScope instruments. If the query is not
              run on the master instrument, data returned indicates that the instrument is running
              as a StandAlone Master. DPO70000SX Series only.

        **Usage:**
            - Using the ``.query()`` method will send the ``IDNMultiscope?`` query.
            - Using the ``.verify(value)`` method will send the ``IDNMultiscope?`` query and raise
              an AssertionError if the returned value does not match ``value``.
...
class DPO70KSXMixin:
    """A mixin that provides access to the DPO70KSX commands and constants.

    Properties:
        - ``.command_argument_constants``: The DPO70KSX command argument constants.
        - ``.commands``: The DPO70KSX commands.
    """

    def __init__(self, *args: Any, **kwargs: Any) -> None:
        super().__init__(*args, **kwargs)
        device = self if isinstance(self, PIDevice) else None
        self._command_argument_constants = DPO70KSXCommandConstants()
        self._commands = DPO70KSXCommands(device)

    @property
    def command_argument_constants(self) -> DPO70KSXCommandConstants:
        """Return the DPO70KSX command argument constants.
    ...
    @property
    def commands(self) -> DPO70KSXCommands:
        """Return the DPO70KSX commands.

        This provides access to all the commands for the DPO70KSX device. See the documentation of
