...
class SourceItemVoltageLevelImmediateBiasEnable(SCPICmdWrite, SCPICmdRead):
    ...
    Usage:
        ...
        - Using the ``.verify(value)`` method will send the
          ``SOURce[n]:VOLTage:LEVel:IMMediate:BIAS:ENABle?`` query and raise an AssertionError if
...
class SourceItemVoltageLevelImmediateBias(SCPICmdWrite, SCPICmdRead):
    ...
    Usage:
        - Using the ``.query()`` method will send the ``SOURce[n]:VOLTage:LEVel:IMMediate:BIAS?``
          query.
        - Using the ``.verify(value)`` method will send the
          ``SOURce[n]:VOLTage:LEVel:IMMediate:BIAS?`` query and raise an AssertionError if the
          returned value does not match ``value``.
        - Using the ``.write(value)`` method will send the
    ...
    def __init__(self, device: Optional["PIDevice"], cmd_syntax: str) -> None:
        ...
        self._enable = SourceItemVoltageLevelImmediateBiasEnable(
    ...
    def enable(self) -> SourceItemVoltageLevelImmediateBiasEnable:
        ...
        Usage:
            - Using the ``.query()`` method will send the
              ``SOURce[n]:VOLTage:LEVel:IMMediate:BIAS:ENABle?`` query.
            - Using the ``.verify(value)`` method will send the
              ``SOURce[n]:VOLTage:LEVel:IMMediate:BIAS:ENABle?`` query and raise an AssertionError
...
class SourceItemVoltageLevelImmediate(SCPICmdRead):
    ...
    Properties:
        - ``.bias``: The ``SOURce[n]:VOLTage:LEVel:IMMediate:BIAS`` command.
    ...
    def __init__(self, device: Optional["PIDevice"], cmd_syntax: str) -> None:
        super().__init__(device, cmd_syntax)
        self._bias = SourceItemVoltageLevelImmediateBias(device, f"{self._cmd_syntax}:BIAS")
...
class SourceItemRmode(SCPICmdWrite, SCPICmdRead):
    ...
    Info:
        - ``CONTinuous`` sets the Run Mode to Continuous (not waiting for trigger). TRIGgered sets
          ...
          waveform plays out continuously. GATed sets the Run Mode to only playout a waveform while
...
class SourceItemCasset(SCPICmdRead):
    ...
    def __init__(self, device: Optional["PIDevice"], cmd_syntax: str) -> None:
        super().__init__(device, cmd_syntax)
        self._clear = SourceItemCassetClear(device, f"{self._cmd_syntax}:CLEar")
...
class SourceItem(ValidatedChannel, SCPICmdRead):
    ...
    def __init__(self, device: Optional["PIDevice"] = None, cmd_syntax: str = "SOURce[n]") -> None:
        ...
        self._cfrequency = SourceItemCfrequency(device, f"{self._cmd_syntax}:CFRequency")
    ...
    def rmode(self) -> SourceItemRmode:
        ...
        Info:
            - ``CONTinuous`` sets the Run Mode to Continuous (not waiting for trigger). TRIGgered
              ...
              received, the waveform plays out continuously. GATed sets the Run Mode to only playout
              a waveform while the trigger is enabled.
            - ``*RST`` sets this to CONT.
        """
        return self._rmode
...
class Source(SCPICmdRead):
    ...
    def __init__(self, device: Optional["PIDevice"] = None, cmd_syntax: str = "SOURce") -> None:
        ...
        self._iqimode = SourceIqimode(device, f"{self._cmd_syntax}:IQIMode")
